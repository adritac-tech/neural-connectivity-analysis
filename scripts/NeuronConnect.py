...
class NeuralAnalysis():
    ...
    def randomization(self, matrix):
        ...
        while num < 1000:
            ...
            if matrix[x, y] != 0 and matrix[x2, y2] != 0 and x != x2 and y != y2 and x != y and x2 != y2:
                ...
                matrix[x2, y2] = b
                num = num + 1
        matrixPandas = pd.DataFrame(matrix, index = self._neurons, columns = self._neurons)   
        return matrixPandas

    def adjacencyMatrix(self):
        output = pd.DataFrame(np.zeros((279,279)), columns = self._neurons, index = self._neurons)

        for i in range(0, len(self._chart)):
            
            if self._chart.iloc[i][2] == 'R':
                value1 = self._chart.iloc[i][0]
                value2 = self._chart.iloc[i][1]
                output.at[value2, value1] = output.at[value2, value1] + self._chart.iloc[i][3]
            elif self._chart.iloc[i][2] == 'Rp':
                value1 = self._chart.iloc[i][0]
                value2 = self._chart.iloc[i][1]
                output.at[value2, value1] = output.at[value2, value1] + self._chart.iloc[i][3]
            elif self._chart.iloc[i][2] == 'S':
                value1 = self._chart.iloc[i][0]
                value2 = self._chart.iloc[i][1]
                output.at[value1, value2] = output.at[value2, value1] + self._chart.iloc[i][3]
            elif self._chart.iloc[i][2] == 'Sp':
                value1 = self._chart.iloc[i][0]
                value2 = self._chart.iloc[i][1]
                output.at[value1, value2] = output.at[value2, value1] + self._chart.iloc[i][3]
            elif self._chart.iloc[i][2] == 'EJ':
                value1 = self._chart.iloc[i][0]
                value2 = self._chart.iloc[i][1]
    ...
    def BinaryCreate(self):
        binaryOutput = pd.DataFrame(np.zeros((279,279)), columns = self._neurons, index = self._neurons)

        #creating binary matrix (still directed)
        for i in range(0, len(self._chart)):
            if self._chart.iloc[i][2] == 'R':
                    if self._chart.iloc[i][3] != 0:
                        value1 = self._chart.iloc[i][0]
                        value2 = self._chart.iloc[i][1]
                        binaryOutput.at[value2, value1] = 1
                    elif self._chart.iloc[i][3] == 0:
                        value1 = self._chart.iloc[i][0]
                        value2 = self._chart.iloc[i][1]
        ...
        return binaryNum
    def InverseCreate(self):
        inverseOutput = pd.DataFrame(np.zeros((279,279)), columns = self._neurons, index = self._neurons)
        for i in range(0, len(self._chart)):
            if self._chart.iloc[i][2] == 'R':
                if self._chart.iloc[i][3] != 0:
                    value1 = self._chart.iloc[i][0]
                    value2 = self._chart.iloc[i][1]
                    inverseOutput.at[value2, value1] = 1/self._chart.iloc[i][3]
                elif self._chart.iloc[i][3] == 0:
                    value1 = self._chart.iloc[i][0]
                    value2 = self._chart.iloc[i][1]
                ...
                if self._chart.iloc[i][3] != 0:
                    ...
                    value2 = self._chart.iloc[i][1]
                    inverseOutput.at[value2, value1] = 1/self._chart.iloc[i][3]
                    inverseOutput.at[value1, value2] = 1/self._chart.iloc[i][3]
                elif self._chart.iloc[i][3] == 0:
                    value1 = self._chart.iloc[i][0]
                    value2 = self._chart.iloc[i][1]
                    inverseOutput.at[value2, value1] = 0
...
if __name__ == "__main__":
    ...
    test_obj = NeuralAnalysis(r'../data/CorrectNeuronConnect.xlsx')
    output = test_obj.adjacencyMatrix()
    outputNumpy = output.to_numpy()
